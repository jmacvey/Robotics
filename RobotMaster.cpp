	...
	byte i2cAddress)
{
	...
	lineFollower = NULL;
	wallFollower = NULL;
	// check if an address was supplied
	//if (i2cAddress != NO_ADDRESS_SUPPLIED) {
	//	lcdManager = new LCDManager(i2cAddress);
	//}
	//else // else try to create a new LCDManager
	//{
...
{
	// STUB
	lineFollower = new LineFollower(motorMaster);
}
bool RobotMaster::initializeWallFollower() {
	// STUB
	wallFollower = new WallFollower(motorMaster);
}
